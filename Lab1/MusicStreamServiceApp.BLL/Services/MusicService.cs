...
namespace MusicStreamServiceApp.BLL.Services
{
    public class MusicService : SetOfFields, IMusicService
    {
        ...
        public async Task AddMusicAsync(MusicCUDTO musicCreateDTO)
        {
            ...
            music.AlbumId = await CheckCatalogAsync(musicCreateDTO.Catalog, musicCreateDTO.Author);
        ...
        public async Task UpdateMusicAsync(int Id, MusicCUDTO musicDTO)
        {
            ...
            music.Id = Id;
            music.AlbumId = await CheckCatalogAsync(musicDTO.Catalog, musicDTO.Author);
        ...
        private async Task<MusicCUDTO> ToFillMusicCUDTORecordsAsync(Music music)
        {
            ...
            {
                var album = await unitOfWork.CatalogRepository.Get(music.AlbumId.Value);

                musicDTO.Catalog = album.Name;
            }
            else
            {
                musicDTO.Catalog = "";
        ...
        private async Task<int?> CheckCatalogAsync(string CatalogName, string AuthorName)
        {
            if (!string.IsNullOrWhiteSpace(CatalogName))
            {
                var album = await unitOfWork.CatalogRepository.Get(CatalogName, AuthorName);
                if (album == null)
                {
